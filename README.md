...
### Linux (Ubuntu / Debian / WSL com interface gráfica)
...
1. Certifique-se de ter o **Python 3.x** instalado no Windows (com `py` ou `python` acessível no PATH).
2. Dentro da pasta do projeto (por exemplo `C:\Users\SeuUsuario\Desktop\trabRedes\trab2Redes`), crie e ative um ambiente virtual e instale as dependências.
...
*(Mesma observação: `pynput` será usado futuramente para o controle de mouse/teclado).*
...
Windows (PowerShell/CMD, com venv ativado)
      py server.py
...
Abra o arquivo `client.py` e, se necessário, edite a variável que representa o IP do servidor (por exemplo `SERVER_IP` ou `SERVER_HOST`):
